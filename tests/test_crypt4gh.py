...
# from crypt4gh import encrypt, decrypt, reencrypt, Header
from crypt4gh import engine, header
...
class TestCrypt4GH(unittest.TestCase):
    ...
    def test_encrypt(self, filedir):
        ...
        with open('file.in', 'rb') as inf, open('file.out', 'wb') as outf:
            engine.encrypt(pubkey, inf, outf)
        compare(dir.read('file.out'), bytes.fromhex(test_gdata.ENC_FILE_HEX))
        filedir.list_dir()
        filedir.cleanup()
